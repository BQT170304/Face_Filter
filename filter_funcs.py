...
config_path = './configs/filters.yaml'
...
def load_filter(filter_name="dog"):
    ...
    for filter in filters:
        ...
        if filter['morph']:
            ...
            dt = tm.calculateDelaunayTriangles(rect, hull)
...
if __name__ == "__main__":
    image = cv2.imread("filters/images/squid_game_front_man.png", cv2.IMREAD_UNCHANGED)
    # print(filters_config['dog'])
    points = load_landmarks(filters_config['anonymous'][0]['anno_path'])
    filters, multi_filter_runtime = load_filter('anonymous')
    print(type(filters), filters)
    # print(type(multi_filter_runtime), len(multi_filter_runtime))
